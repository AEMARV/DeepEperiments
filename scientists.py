...
if __name__ == '__main__':
	...
	for i in np.arange(400):
		for sci_idx_main in np.arange(sci_number):
			...
			for sci_idx_partner in np.arange(sci_number):
				if sci_idx_partner == sci_idx_main: continue
				load_scientist_model(sci_idx_partner, training_model_a)
				transformed_dataset = transform_dataset(corrupt_model_a,vanil_dataset_sliced[sci_idx_main],label_sliced[sci_idx_main],5,i,
				                                        training_model_a)
				prepared_dataset_list += [transformed_dataset]
				prepared_label_list += [label_sliced[sci_idx_main]]
			...
			training_model_a.fit_generator(
				data_gen.flow(prepared_dataset, prepared_label, batch_size=opts['training_opts']['batch_size'], shuffle=True, seed=opts['seed']),
				samples_per_epoch=samples_per_epoch, nb_epoch=5, callbacks=callback_list, validation_data=(data_test, label_test))
