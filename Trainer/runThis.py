...
def wrapper_gated(model,opts,experiment_name):
	opts['experiment_name']= experiment_name
	method_names = find_key_value_to_str_recursive(opts,'',{'stoch','w_regularizer','gate_activation',
	                                                        'data_activation'})
...
if __name__ == '__main__':
	models= ['gatenet_binary_model','gatenet_binary_merged_model','lenet_amir']
	datasets=['cifar10','cifar100']
	experiment_name = 'Test'+time.strftime('%b%d')

	for dataset_str in datasets:
		for model_str in models:
			options={}
			options = default_opt_creator()
			options['description'] = 'Creating Final Baseline For three Models'
