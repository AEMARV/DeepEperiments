...
def baseline2_besh12(opts, input_shape, nb_classes, getstring_flag=False):
	model_string = 'besh|f:32,r:5,p:1' \
	               ...
	               '->leaffully|u:1,n:16,ido:-1' \
	               '->ap|s:2,r:3' \
	               '->shdensedoi|n:-1,tdo:.5,ido:-1,m:0'
	nb_filter_list = [32, 32, 64, 128, 64, 64]
	filter_size_list = [5, 5, 3, 5, 3, 5, 3, 4, 3]
	if getstring_flag:
		return {'string': model_string, 'nb_filter': nb_filter_list, 'filter_size': filter_size_list}
...
def nin_besh_caffe3(opts, input_shape, nb_classes, getstring_flag=False):
	...
	model_string = 'convsh|f:192,r:5,l2_val:1e-4->crelu' \
	               ...
	               '->convsh|f:' + str(nb_classes) + ',r:1->relu' \
	                                                 ...
	                                                 '->softmax->fin'
	return get_model(opts, input_shape, nb_classes, model_string=model_string)
def nin_besh_caffe4(opts, input_shape, nb_classes, getstring_flag=False):
	# Same Structure as nin besh 1 2 3
	model_string = 'convsh|f:192,r:5,l2_val:1e-4->ber' \
	               ...
	               '->convsh|f:' + str(nb_classes) + ',r:1->relu' \
	                                                 ...
	                                                 '->softmax->fin'
	return get_model(opts, input_shape, nb_classes, model_string=model_string)
def nin_besh_caffe5(opts, input_shape, nb_classes, getstring_flag=False):
	# Same Structure as nin besh 1 2 3
	model_string = 'convsh|f:192,r:5,l2_val:1e-4->ber' \
	               '->convsh|f:160,r:1,l2_val:1e-4->ber' \
	               '->convsh|f:96,r:1,l2_val:1e-4->ber' \
...
def get(identifier):
    if isinstance(identifier, dict):
        ...
        raise ValueError('Could not interpret model identifier:',
                         identifier)
