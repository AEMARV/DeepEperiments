...
def vgg_gated(include_top=True,weights=None,input_tensor=None,opts=None,initialization='glorot_normal'):
	...
	model.summary()
	return model
def gated_lenet(opts, input_shape, nb_classes,
                input_tensor=None,
	...
	average_reg_dict = opts['model_opts']['param_dict']['gate_layer']['gate_activity_regularizer']['param_dict'][
		'average_reg']
	average_reg_weight_dict = opts['model_opts']['param_dict']['gate_layer']['gate_activity_regularizer']['param_dict'][
		'average_reg_weight']
	...
	x = Convolution2D(32*expand_rate, 3, 3, activation=data_activation, input_shape=input_shape,
	              border_mode='same', W_regularizer=w_reg)(img_input)
	x = Convolution2D(32 * expand_rate, 3, 3, activation=data_activation,  border_mode='valid',
	                  W_regularizer=w_reg)(x)
	x = MaxPooling2D(pool_size=(2, 2))(x)
	# x = Dropout(0.25)(x)

	x = gate_layer(x, int(64*expand_rate/2), 3,input_shape=(32,(input_shape[1]-2)/2,(input_shape[2]-2)/2),opts=opts,
	               border_mode='same',activity_reg_weight=average_reg_weight_dict[2],average_reg=average_reg_dict[2])

	x = gate_layer(x, int(64*expand_rate/2), 3,input_shape=(64,((input_shape[1]-2)/2)-2,((input_shape[2]-2)/2)-2),
	               opts=opts,
	               border_mode='valid',activity_reg_weight=average_reg_weight_dict[3],average_reg=average_reg_dict[3])
	# x = Activation('relu')(x)
	x = MaxPooling2D(pool_size=(2, 2))(x)
	# x = Dropout(0.25)(x)
	if not include_top:
