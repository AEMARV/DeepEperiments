...
def kl_both_centric(cross_xprob_kerlog, cross_xlog_kerprob, ent_x, ent_ker):
	distance = 0
...
def kl_v2(cross_xprob_kerlog, cross_xlog_kerprob, ent_x, ent_ker):
	...
	#distance += ent_ker
	return distance
def kl_data_centric(cross_xprob_kerlog, cross_xlog_kerprob, ent_x, ent_ker):
	distance = 0
	distance += cross_xlog_kerprob
	#distance += cross_xprob_kerlog
	#distance -= ent_x
	distance += ent_ker
	return distance
def kl_model_centric(cross_xprob_kerlog, cross_xlog_kerprob, ent_x, ent_ker):
	distance = 0
	#distance += cross_xlog_kerprob
	distance += cross_xprob_kerlog
	distance += ent_x
...
def kl_loss_model_centric(y_true,y_pred):
	...
	loss += ent_preds
	return -loss
def kl_loss_data_centric(y_true,y_pred):
	loss = 0
	y_true_cut = K.clip(y_true, K.epsilon(), 1-K.epsilon())
	y_true_cut = y_true_cut/K.sum(y_true_cut,axis=[-1],keepdims=True)
	ent_preds = -K.exp(y_pred)*y_pred
