...
def helloKl_Concentrated_Widev1(opts, input_shape, nb_classes, getstring_flag=False):
    ...
    opts['optimizer_opts']['momentum'] = 0.9
    return get_model_out_dict(opts, model_string=model_string)
# Natural Concentration ''' Here '''
def helloKl_nat(opts, input_shape, nb_classes, getstring_flag=False):
    # Same Structure as nin besh 1 2 3
    regklb = None
    distvec=[]
...
def helloKl_nat_conc(opts, input_shape, nb_classes, getstring_flag=False):
    # Same Structure as nin besh 1 2 3
    regklb = None
    distvec=[]
    model_string = 'klconvbnat|f:32,r:5,l2_val:5e-4,bias:0->lsoft' \
                   '->klavgpool|r:3,s:2' \
                   '->klconvnat|f:64,r:5,l2_val:1e-4,bias:0->lsoft' \
                   '->klavgpool|r:3,s:2' \
                   '->klconvnat|f:128,r:3,l2_val:1e-4,bias:0->lsoft' \
                   '->klavgpool|r:3,s:2' \
                   '->klconvnat|f:192,r:1,l2_val:1e-4,bias:0->lsoft' \
                   '->klconvnat|f:' + str(nb_classes) + ',r:1->lsoft|reg:0' \
                                                     '->klavgpool|r:3,s:1' \
                                                     '->flattensh' \
                                                     '->fin'
    use_link_func = False
