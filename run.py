...
def check_model_list(model_list, datasets):
	...
	for dataset_str in datasets:
		for model in model_list:
			...
			model = get_model_from_db(model, opts)


if __name__ == '__main__':
	# gatenet_binary_merged_model lenet_amir ,gatenet_binary_model
	models = ['nin_tree_structure_2_perm_rand_1', 'nin_tree_structure_2_perm_rand_activity_1']
	...
	for dataset_str in datasets:
		...
		for model_str in models:
			try:
				...
				model.summary()
				# model_modification_utils.load_weights_by_block_index_list(model, [1, 2, 3],
				#                                                           os.path.join(global_constant_var.get_experimentcase_abs_path(
				# experiment_name,
				# 	                                                          dataset_str, model_str), 'checkpoint'),
				# model_constructor_utils.CONV_NAME)
				...
				print '{} model had an exception'.format(model_str)
